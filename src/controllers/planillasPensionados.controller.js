const service = require("../services/planillasPensionados.service");
const { successResponse } = require("../utils/response");

const list = async (req, res, next) => {
  try {
    const data = await service.list();
    return successResponse(res, data, "Planillas de pensionados obtenidas correctamente");
  } catch (e) { next(e); }
};
...
const generar = async (req, res, next) => {
  try {
    ...
    const data = await service.generar(req.params.id, tipoIngreso, req.user);
...
const reversarJubilado = async (req, res, next) => {
  try {
    const data = await service.reversarJubilado(req.params.id, req.params.jid, req.body.motivo, req.user);
    return successResponse(res, data, "PAGO DE PENSIONADO REVERSADO CORRECTAMENTE");
  } catch (e) { next(e); }
};
...
const estadoCuenta = async (req, res, next) => {
  try {
    ...
    return successResponse(res, data, "Estado de cuenta obtenido correctamente");
  } catch (e) { next(e); }
};

const generarDeudaHistoricaMasivo = async (req, res, next) => {
  try {
    const { periodoFinal, porcentaje } = req.body;
    const data = await service.generarDeudaHistoricaMasivo(periodoFinal, porcentaje, req.user);
    return successResponse(res, data, "Deuda histórica generada correctamente");
  } catch (e) { next(e); }
};
