...
const catalogs = {
  ...
  "tipo-planilla": {
    ...
    toResponse: (row) => ({ id: row.tpl_id, tipoPlanilla: row.tpl_tipo_planilla, descripcion: row.tpl_descripcion, idTipoUso: row.tpl_id_tipo_uso, fechaCreacion: row.tpl_fecha_creacion, usuarioCreacion: row.tpl_usuario_creacion })
  },
  "parametro-general": {
...
const create = async (catalogKey, payload, currentUser) => {
  const catalog = getCatalog(catalogKey);
  validatePayload(catalog, payload);
  const createdBy = currentUser?.usuario || "sistema";
  const [result] = await pool.execute(getQuery(catalog, "crear"), [...catalog.toDb(payload), createdBy]);
...
const update = async (catalogKey, id, payload, currentUser) => {
  ...
  await pool.execute(getQuery(catalog, "actualizar"), [...catalog.toDb(payload), id]);
