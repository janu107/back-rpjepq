SELECT
  (SELECT COUNT(*) FROM RPJ_MNT_EMPLEADO
   WHERE emp_tipo_manejo = 1 AND UPPER(COALESCE(emp_estado,'')) = 'ACTIVO')                                AS total_activos,
  (SELECT COUNT(DISTINCT d.dat_id_empleado)
   ...
   WHERE d.dat_tipo_manejo = 1 AND d.dat_aplica_nomina = 1 AND UPPER(COALESCE(e.emp_estado,'')) = 'ACTIVO') AS con_datos_planilla,
